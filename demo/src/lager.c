  ...
  do
    {
      ...
      if (index_to_remove == INVALID_INDEX) printf("Ogiltigt val!\n");
...
{               //Heltals-motsvarigheten till '1'
  int *menu_choices = (int*)allocate_array(item_count, sizeof(int*), NULL); // Char-array för alla index med två extra platser för 'A' och 'a'
  retain(menu_choices);
