...
obj constructor_allocate_tester(size_t bytes, function1_t destructor, size_t refc)
{
  set_cascade_limit(CASCADE_LIMIT); 
  object_record_t *record = malloc(sizeof(*record) + bytes);

  record->ref_count = refc; // To be able to test the refcounter.
  record->destroy   = destructor ? destructor : no_destructor;
  record->next      = heap_allocations;
  record->check     = checkstring;
  heap_allocations  = record;
...
{
  size_t total = abs(elements * elem_size);
