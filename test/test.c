...
retain_test (void)
{
    for (int i = 0; i < 4; i++)
        {
            obj tmp_object = constructor_allocate_tester ((size_t) 32, NULL,
                             different_rc[i]);
            retain (tmp_object);
...
main (int argc, char *argv[])
{
    ...
    if (
        ...
        (NULL == CU_add_test (MARC_suite, "set_cascade_limit",
                              set_cascade_limit_test)) ||
        (NULL == CU_add_test (MARC_suite, "get_cascade_limit",
                              get_cascade_limit_test)) ||
